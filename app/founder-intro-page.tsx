...
export function FounderIntroPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/cms/content")
      ...
      .then((json: { entries?: Array<{ collection: string; title: string; excerpt: string; imageUrl: string; price?: string; tag?: string; content: string }> }) => {
        if (!active || !Array.isArray(json.entries)) return;
        const founderEntry = json.entries.find((e) => e.collection === "founder-intro");
        if (founderEntry) {
          setData({
            title: founderEntry.title || defaultFounderData.title,
            excerpt: founderEntry.excerpt || defaultFounderData.excerpt,
            imageUrl: founderEntry.imageUrl || defaultFounderData.imageUrl,
            price: founderEntry.price || defaultFounderData.price,
            tag: founderEntry.tag || "dang-cap-nhat",
            content: founderEntry.content || defaultFounderData.content,
          });
        }
      })
      .catch(() => undefined);
  ...
  // Use the new portrait photo as requested
  const displayImage =
    data.imageUrl &&
    data.imageUrl !== "/intro-portrait.jpg" &&
    !data.imageUrl.includes("/intro-portrait.jpg|")
        ...
        <div className="py-10 sm:py-16 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div className="text-center space-y-4 max-w-3xl mx-auto">
            ...
            <p className="text-base sm:text-lg text-[#5C4D47] leading-relaxed">
              ...
              <span className="text-[#70141D] font-bold">Thầy Quách Hạ Văn</span> với tâm nguyện gìn giữ, phát triển và xây dựng hệ thống học sáo trúc bài bản, truyền cảm hứng cho người yêu âm nhạc dân tộc.
            </p>
          </div>

          {/* Conditional View: ĐANG CẬP NHẬT vs HIỂN THỊ ĐẦY ĐỦ */}
          {isUpdating ? (
            /* ============================================================
               VIEW 1: TRẠNG THÁI ĐANG CẬP NHẬT (Theo yêu cầu của Admin)
               ============================================================ */
            <div className="bg-gradient-to-br from-[#FAF6F0] via-[#F6ECE0] to-[#EADBCA] rounded-3xl p-6 sm:p-10 border-2 border-[#D9C4A6] shadow-xl space-y-8 relative overflow-hidden">
              {/* Header Seal & Tag */}
              <div className="flex items-center justify-between">
                <span className="text-xs uppercase tracking-widest text-amber-900 font-bold bg-amber-500/15 border border-amber-600/30 px-4 py-1.5 rounded-full flex items-center gap-2 shadow-2xs">
                  <i className="fa-solid fa-clock-rotate-left text-amber-800"></i> Thông Tin Đang Cập Nhật
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
                {/* Left: Founder Single Portrait */}
                <div className="md:col-span-4 flex flex-col items-center text-center space-y-3">
                  <div className="relative group">
                    <div className="w-40 h-40 sm:w-44 sm:h-44 rounded-3xl overflow-hidden shadow-xl border-4 border-white bg-[#EDE4D8]">
                      <img
                        src={displayImage}
                  ...
                  <div>
                    ...
                    <span className="inline-block text-xs text-[#6B5A53] mt-1 font-semibold">
                      Hotline / Zalo: {data.price || "0374 261 368"}
                    </span>
                  </div>
                </div>
                ...
                <div className="md:col-span-8 space-y-5">
                  <div className="bg-white/95 rounded-2xl p-6 border border-[#D9CDBB] shadow-sm space-y-3">
                    <div className="flex items-center gap-2 text-[#70141D] font-bold text-sm uppercase tracking-wide">
                      <i className="fa-solid fa-pen-nib text-amber-700"></i>
                      <span>Thông Báo Cập Nhật Nội Dung</span>
                    </div>
                    <p className="text-sm sm:text-base text-[#59524B] leading-relaxed">
                      {data.excerpt || "Hệ thống thông tin chi tiết về hồ sơ nghệ thuật, phương pháp đào tạo và các dấu ấn chuyên môn của Thầy Quách Hạ Văn đang trong quá trình hoàn thiện nội dung."}
                    </p>
                    <p className="text-xs sm:text-sm text-[#6B625B] leading-relaxed">
                      Chúng tôi sẽ sớm ra mắt nội dung chi tiết nhất để phục vụ quý học viên và bạn yêu âm nhạc dân tộc. Trong thời gian này, mọi thông tin tư vấn lớp học, giáo trình và sáo trúc chuẩn âm xin vui lòng liên hệ trực tiếp qua số hotline/Zalo bên dưới.
                    </p>
                  </div>

                  {/* Quote Box */}
                  <div className="p-4 sm:p-5 bg-white/70 rounded-2xl border border-[#E0D5C3] italic text-xs sm:text-sm text-[#292421] leading-relaxed">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
                    <a
                      href={`https://zalo.me/${(data.price || "0374261368").replace(/\D/g, "")}`}
                      target="_blank"
                    ...
                    >
                      ...
                      <span>Liên hệ Zalo Thầy Văn ({data.price || "0374 261 368"})</span>
                    </a>
                    <Link
                      href="/dang-ky-hoc"
                      className="w-full sm:w-auto px-6 py-3.5 bg-white hover:bg-[#FAF6EE] text-[#292421] border border-[#D9CDBB] text-xs font-bold rounded-xl shadow-xs transition-all flex items-center justify-center gap-2"
                    >
                      <i className="fa-solid fa-graduation-cap"></i>
                      <span>Đăng ký tư vấn lớp học</span>
                    </Link>
                  </div>
                </div>
              </div>
            ...
            /* ============================================================
               VIEW 2: HIỂN THỊ ĐẦY ĐỦ (Khi Admin kích hoạt)
               ============================================================ */
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              ...
              <p className="text-xs sm:text-sm text-[#59524B] leading-relaxed">
                ...
                📞 Hotline / Zalo Thầy Văn: <strong>{data.price || "0374 261 368"}</strong> • ✉️ Email: <strong>saotrucauco@gmail.com</strong>
              </p>
            </div>
