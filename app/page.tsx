  ...
  return (
    <main>
      ...
      {activeService === "classes" && <section className="courses section" id="classes">
        ...
        <div className="discipline-grid">{displayedDisciplines.map((item, i) => <article className={openDiscipline === i ? "discipline-card is-open" : "discipline-card"} key={item.title}><button className="discipline-summary" onClick={() => setOpenDiscipline(openDiscipline === i ? null : i)} aria-expanded={openDiscipline === i}><span className="discipline-photo"><img src={item.image} alt={item.imageAlt} width="640" height="420" loading="lazy" decoding="async" /><i>{item.icon}</i></span><span className="discipline-copy"><small>{t("BỘ MÔN", "DISCIPLINE")} 0{i + 1}</small><h3>{item.title}</h3><p>{item.short}</p></span><b>{openDiscipline === i ? t("Thu gọn −", "Collapse −") : t("Xem nhanh +", "Quick view +")}</b></button>{openDiscipline === i && <div className="discipline-detail"><p>{item.intro}</p><div><span><small>{t("NỘI DUNG HỌC", "SYLLABUS")}</small><ul>{item.learn.map((point) => <li key={point}>{point}</li>)}</ul></span><span><small>{t("PHÙ HỢP VỚI", "SUITABLE FOR")}</small><p>{item.suitable}</p></span></div><div className="discipline-actions"><a className="article-link" href={`/bo-mon/${item.slug}`}>{t("Xem bài giới thiệu đầy đủ →", "Read full discipline guide →")}</a><button className="button button-wine" onClick={() => { setSelectedDiscipline(item.title); openService("#contact"); }}>{t("Đăng ký bộ môn này", "Enroll in this course")}</button></div></div>}</article>)}</div>
      </section>}
