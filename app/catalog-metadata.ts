...
export async function catalogMetadata(params: Promise<{ slug: string }>, typeLabel: string, basePath: string): Promise<Metadata> {
  ...
  const description = `${typeLabel} ${title} tại Sáo Trúc Âu Cơ. Xem mô tả chi tiết, giá và gửi yêu cầu tư vấn.`;
  return {
    title,
    description,
    alternates: { canonical: `/${basePath}/${slug}` },
    openGraph: { type: "website", locale: "vi_VN", siteName: "Sáo Trúc Âu Cơ", title, description, url: `/${basePath}/${slug}` },
  };
}
